...
export function UserContactLayout(props: UserContactProps): JSX.Element {
  return (
    <article className='py-3 px-5 interface flex items-center cursor-pointer justify-between rounded-2xl duration-300 transition-colors hover:bg-inter group/user'>
      <div className='flex items-center'>
        <div className='w-16 h-16 relative bg-radial-[at_25%_25%] from-accent to-accent-shadow to-75% rounded-full flex items-center justify-center'>
          {(props.image && (
            <img
              src={props.image}
              alt={props.name}
          ...
          {props.online && (
            <div className='w-4 h-4 bg-accent absolute bottom-1 right-2 border-background rounded-full border-2 duration-300 transition-colors group-hover/user:border-inter'></div>
          )}
        </div>
        <div className='ml-4'>
